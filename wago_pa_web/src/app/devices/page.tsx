      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-6xl">
        ...
        {devices.map(device => (
          <div key={device.id} className="bg-white rounded shadow p-6 flex flex-col gap-2 border border-gray-200">
            <div className="font-semibold text-lg text-[#28a745]">{device.name}</div>
            ...
            <Link href={{ pathname: '/device-detail', query: { id: device.id } }} legacyBehavior>
              <a className="mt-2 px-4 py-2 rounded bg-[#28a745] text-white hover:bg-[#6ec800] transition text-center">View Details</a>
            </Link>
          </div>
        ))}
      </div>
