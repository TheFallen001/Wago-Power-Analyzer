...
// Mock Data
const pieData = [
  { name: 'Off Hours', population: 34.1, color: '#60A5FA', legendFontColor: '#1E293B', legendFontSize: 14 },
  { name: 'Auxiliary', population: 65.9, color: '#34D399', legendFontColor: '#1E293B', legendFontSize: 14 },
];
...
const devicePieData = deviceList.map((name, idx) => ({
  name,
  data: [
    { name: 'Off Hours', population: Math.floor(Math.random() * 60) + 20, color: '#60A5FA', legendFontColor: '#1E293B', legendFontSize: 12 },
    { name: 'Auxiliary', population: Math.floor(Math.random() * 60) + 20, color: '#34D399', legendFontColor: '#1E293B', legendFontSize: 12 },
  ],
}));
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.fullRow}>
        <View style={styles.fullCard}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <BarChart
              data={{ labels: barData.labels, datasets: barData.datasets.map(ds => ({ data: ds.data })) }}
              width={Math.max((barData.labels.length * 120), screenWidth - 32)}
              height={220}
              chartConfig={{
                ...chartConfig,
                propsForLabels: { fontWeight: 'bold', fontSize: 12, rotation: 0 },
                formatXLabel: (label: string) => label.length > 12 ? label.slice(0, 12) + '…' : label,
              }}
              fromZero
              showBarTops={false}
      ...
      <View style={styles.fullRow}>
        <View style={styles.fullCard}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <BarChart
              data={{ labels: barData2.labels, datasets: barData2.datasets.map(ds => ({ data: ds.data })) }}
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }}>
        {devicePieData.map((dev, i) => (
          <View key={i} style={[styles.deviceCard, { width: 180, marginRight: 16 }]}> 
